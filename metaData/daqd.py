...
from petsys import fe_temperature
...
class Connection:
	...
	# @param metadata Metadata class object containing run parameters to be saved to file
	# @param tempInterval Time interval in seconds between temperature readings (added by Wilf; June 2022)
	def acquire(self, acquisitionTime, step1, step2, metadata, tempInterval=300):
		...
		while currentFrame < stopFrame:
			if (currentFrame-startFrame)%tempIntervalFrames == 0:
				runtime = (currentFrame-startFrame)/tempIntervalFrames*tempInterval
				# read temp sensors
				for sensor in sensor_list:
					...
					metadata.run_temp[location][time] = sensor.get_temperature()
			wrPointer, rdPointer = self.__getDataFrameWriteReadPointer()
			while wrPointer == rdPointer:
				wrPointer, rdPointer = self.__getDataFrameWriteReadPointer()
